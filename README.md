...
This project develops a dual-phase supervised machine learning framework to accurately classify households' Below Poverty Line (BPL) status and detect potential welfare misrepresentation. Here one will not find the data in the excel files as they are not to be shared!! Also, due to upload constraints, the model weights and all the image datasets cannot be uploaded here. So, for OPENAI's CLIP model download the specific files mentioned in the particular folder and run the model training file in Google Collab to get the final weights of the image recognition model!!  
